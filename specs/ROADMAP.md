...
| **Inventario** | Salida ligada a historial para medicamentos controlados; ~~política mermas~~ **Hecho (MVP 007)**; ~~Proveedores visibles en menú~~ **026**; ~~sub-ítems inventario en sidenav~~ **027**; ~~**043** foto + QR~~ **done**; ~~**044** picker producto~~ **done** — **045** grid catálogo (in_progress) | Media |
| **Portal / auth** | ~~Registro self-service landing~~ **013**; ~~UI perfil dual~~ **012**; ~~desvincular dual~~ **015**; ~~revocación sesiones~~ **hecho + deploy OK**; ~~query mascotas cliente_id~~ **020**; ~~portal baños~~ **028**; ~~interconexión + pensión/recordatorios portal~~ **031**; **047** enlace portal↔ficha clínica (ola 1 ficha + ola 2 self-reg sin duplicar) — in_progress; **Cypress portal** guest + auth (skip sin env) | Alta |
| **Ops / enlaces** | **031 done:** prefill baños, expediente→cita/pensión, cita→caja, stock→OC, KPI links, FCM token SW-ready | Alta — **cerrada** |
| **Finanzas / CxC** | **032/036 done.** **045** hub ticket (cuenta del día + pendientes baño + venta desde ticket) — in_progress. Walk-in petshop → **046**. | Alta |
| **UX admin** | **046** draft: flujos guiados “te falta X”, empty states, sensación móvil, venta mostrador sin cliente — `specs/046-ux-intuitiva-guiada/` | Alta — **prioridad clínica** |
