...
import {
  ...
  ADMIN_DIALOG_DETAIL,
  ADMIN_DIALOG_FORM,
} from '../core/config/admin-ui.config';
import { filtrarClientes, getClienteNombreCompleto } from '../core/utils/cliente-search.util';
import { calcularClienteEstadisticas, calcularClientesConPacientes } from '../core/utils/entity-stats.util';
...
export class ClientesComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit(): void {
    this.route.queryParamMap.pipe(takeUntil(this.destroy$)).subscribe((params) => {
      this.soloConDeuda = params.get('deuda') === '1';
  ...
  portalChipLabel(cliente: any): string {
    const mail = String(cliente?.portalEmail || cliente?.correo || '')
      .trim()
      .toLowerCase();
    ...
    dialogRef
      ...
      .subscribe((result) => {
        if (result && !modoVer) {
          ...
          this.clientesService
            .guardarCliente(result)
            .then(async (id) => {
              const saved = { ...result, id };
              this.todosLosClientes = [saved, ...this.todosLosClientes.filter((c) => c.id !== id)];
              this.aplicarFiltroSucursal();
              this.cargarEstadisticas();
        ...
        this.clientesService
          ...
          .then((res) => {
            ...
            const detalle = [
              ...
              res.portalRevocado
                ? 'portal revocado'
                : res.portalRevokeError
                  ? 'portal desactivado (revocación Auth pendiente)'
                  : 'sin portal',
            ].join(', ');
            setTimeout(() => {
    ...
    this.pacientesService
      ...
      .subscribe({
        next: (pacientes) => {
          ...
          const clientesSinPacientes = this.todosLosClientes.filter(
            (cliente) => !clientesConPacientesSet.has(cliente.id)
          );
