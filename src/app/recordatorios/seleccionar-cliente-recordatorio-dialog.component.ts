...
@Component({
  ...
  templateUrl: './seleccionar-cliente-recordatorio-dialog.component.html',
  styleUrls: ['./seleccionar-cliente-recordatorio-dialog.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class SeleccionarClienteRecordatorioDialogComponent implements OnInit {
  clientes: any[] = [];
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<SeleccionarClienteRecordatorioDialogComponent>,
  ...
  cargarClientes() {
    this.loading = true;
    console.log('🔄 Cargando clientes para recordatorio...');
  ...
  cargarPacientes(clienteId: string) {
    this.loading = true;
    console.log('🔄 Cargando pacientes para recordatorio del cliente:', clienteId);
  ...
  seleccionarPaciente(paciente: any) {
    ...
    if (paciente.activo === false || paciente.estado === 'Fallecido' || paciente.estado === 'fallecido') {
      console.log('❌ No se puede crear recordatorio para paciente inactivo o fallecido:', paciente.nombre);
      return;
    }
  ...
  continuar() {
    if (this.clienteSeleccionado && this.pacienteSeleccionado) {
      ...
      if (this.pacienteSeleccionado.activo === false || 
          ...
          this.pacienteSeleccionado.estado === 'fallecido') {
        console.log('❌ No se puede crear recordatorio para paciente inactivo o fallecido:', this.pacienteSeleccionado.nombre);
        return;
      }
